...
def main(stdscr):
    ...
    for operation in operations:
        ...
        if operation == 'g':
            ...
            input_window.addstr(8, 0, "SBR Upstream or Downstream (U/D): ")
            sbrlocation = input_window.getstr().decode()
            
            root_port_list = []
            for slot in slotlist:
                for gpu_info in gpu_info_list:
                    if gpu_info[1] == slot:
                        root_port = gpu_info[2]
                        root_port_list.append(root_port)
    ...
    if 's' in operations:
        ...
        # device_window.refresh()
        #pad_pos = functions.output_print(output_window, output_window_left_corner[1], output_window_left_corner[0]+2, output_window_height, output_window_width, pad_pos, "Error reporting set to 0.")
        if sbrlocation == "u" or "U":
            sbr.run_test(user_password, 2*inputnum_loops+1, kill, root_port_list, output_window, output_window_left_corner[1], output_window_left_corner[0]+2, output_window_height, output_window_width, pad_pos)
        else:
